...
def _load_hf_timeline(hf_data_path: str = "/mnt/data/upcast/data/trend_analysis/model_cards_with_metadata/train") -> "pd.DataFrame":
    ...
    df = table.select(["modelId", "createdAt", "last_modified"]).to_pandas()
...
def test_arxhf_sampler():
    ...
    ahs = Arxiv_HF_datasets()
    ahs.prepare(threshold=1517)
    hf3_, arx3_ = ahs.sample_subsets(3)
    assert len(hf3_) == 3*32
    #assert hf3_.index.isin(hf3.index).all() # this does not hold
    #assert arx3_.index.isin(arx3.index).all()
